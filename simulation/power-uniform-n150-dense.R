...
n = 150
p = c(500, 1000, 1500)
signal.strength = seq(0.09, 0.21, 0.03)
rep = 1000
m = 0
...
for (i1 in 1 : length(p)){
  ...
  signal.sparsity = 2 * p0
  for(i in 1 : length(signal.strength)){
    for (j in 1 : length(signal.sparsity)){
      Sigma = Cov.alternative.rho.1(p0, sigma02, signal.strength[i], signal.sparsity[j])
...
write.csv(Res, "power-uniform-n150-dense.csv", row.names = FALSE)
